...
void List::DelRange(int pos1, int pos2)
{
	...
	Elem* DelEnd = DelStart;
	while (i <= pos2)
	{
		DelEnd = DelEnd->next;
		i++;
	}
	Elem* PrevDelStart = DelStart->prev;
	Elem* AfterDelEnd = DelEnd->next;
	if (PrevDelStart != 0 && Count != 1)
		PrevDelStart->next = AfterDelEnd;
	if (AfterDelEnd != 0 && Count != 1)
		AfterDelEnd->prev = PrevDelStart;
	if (pos1 == 1)
		Head = AfterDelEnd;
	if (pos2 == Count)
		Tail = PrevDelStart;
	while (DelStart != AfterDelEnd)
	{
		Elem* temp = DelStart;
		DelStart = DelStart->next;
		delete temp;
